//
//  CostsEditInTableViewController.swift
//  MoneyDay
//
...
class CostsEditInTableViewController: UIViewController {
    
    let textFieldCategory = CostsEditeCategoryTextField()
    let labelCategory = CostsEditLabelCategory()
    let btnBack = CostsBtnBack()
    let moneyTextField = CostsEditTextFieldMoney()
    let dateTextField = CostsEditDateTextField()
    let labelDate = CostsEditLabelDate()
    let labelMoney = CostsEditLabelMoney()
    let labelComment = CostsEditLabelComment()
    let textViewComment = CostsEditTextView()
    let buttonAdd = CostsBtnAdd()
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        textFieldCategory.Category = ["Гигиена","Жилье","Еда","Здоровье","Кафе","Машина","Одежда","Питомцы","Подарки","Развлечения","Связь","Спорт"]
    ...
    @objc func tapButtonAdd (_ sender: UIButton) {
        ...
        if !(moneyTextField.text?.isEmpty)! {
            ...
            let option = OptionListObjectCosts(date: dateTextField.picker.date, money: Int(moneyTextField.text!)!, totalScore: Double(moneyTextField.text!)!, comment: textViewComment.text, category: textFieldCategory.text!)
            
            realmMethods.addInRealmCosts(option)
