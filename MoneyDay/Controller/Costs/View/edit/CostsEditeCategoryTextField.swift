...
class CostsEditeCategoryTextField: UITextField, UIPickerViewDelegate, UIPickerViewDataSource {

    let addNewCategoryBtn = UIButton()
    let pickerCategory = UIPickerView()
    var Category = ["Гигиена","Жилье","Еда","Здоровье","Кафе","Машина","Одежда","Питомцы","Подарки","Развлечения","Связь","Спорт"]
